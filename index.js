...
// ANSWER:
// if array never changed then it would be 0(1) but if it does change then it is 0(n)
...
// ANSWER: 0(1). We can write a for loop to make it faster 
...
function squareEveryDigit(number) {
  let string = number.toString().split("").map(x => x * x).join('')
    return string
  }
squareEveryDigit(946) // When running 946, you should see a result of 811636

// This is linear because it's going to map over whatever the number is in 'x'
